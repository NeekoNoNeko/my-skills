---
name: excel-to-anki
description: Use when generating Anki flashcards from Excel word lists with TTS audio — reads .xlsx with 单词/释义/phonetic columns, filters by "known=Selected", generates MP3 via edge-tts, and packs into .apkg
---
...
从 Excel 单词表批量生成带音频的 Anki `.apkg` 卡组。调用 edge-tts 并行生成美式发音 MP3，嵌入卡片后自动播放。

核心流程：`Excel → 过滤 known=Selected → 并行 TTS → 打包 .apkg`

## When to Use

- 有结构化 Excel 单词表（列：单词、词性、释义、音标、短语、例句）
- 需要为每个单词生成真人发音并嵌入 Anki 卡片
- 单词量较大（数百到上千），需要批量并行处理

**前置条件：**
```
pip install edge-tts genanki openpyxl
```

## The Script

保存为项目根目录的 `generate_anki.py`，直接运行：
...
| 例句 | - | 例句，背面有条件显示 |
| 例句释义 | - | 例句翻译 |
| known | ✅ | 仅 `Selected` 的行生成卡片 |

## 自定义

- **更换 TTS 音色**：修改 `VOICE` 变量，用 `edge-tts --list-voices` 查看可用音色
- **调整并发数**：修改 `BATCH_SIZE`（8 = 稳定，16 = 更快但可能被限流）
- **修改卡片模板**：编辑 `FRONT` / `BACK` / `CARD_CSS`

## 常见问题

| 问题 | 解决 |
|------|------|
| `ModuleNotFoundError` | `pip install edge-tts genanki openpyxl` |
| TTS 生成失败 | 检查网络连接，降低 `BATCH_SIZE` |
| 音频不播放 | 确认 `.apkg` 中嵌入了 `[sound:...]` 标签且 mp3 在 media 列表 |
