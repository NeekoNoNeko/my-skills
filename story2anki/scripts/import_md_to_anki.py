...
def add_notes(deck_name, model_name, notes, target_tags):
    added = 0
    skipped = 0
    for i, note in enumerate(notes, 1):
        ...
        print(f"  [{i}/{len(notes)}] {status}: {note['front']}")
...
def main():
    ...
    ensure_model(deck_name)

    print(f"Importing to deck '{deck_name}'...")
    added, skipped = add_notes(deck_name, deck_name, notes, extra_tags)
